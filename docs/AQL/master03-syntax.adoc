...
Aggregate functions calculate a single result from a set of _input values_, aggregated from an _expression_ over the result set.
These expressions are using <<_aql_identified_paths, identified paths>>
so that the functions operate on a set of <<_built_in_types,built in (primitive)>> type values.
Some of these functions support the `DISTINCT` keyword to consider only distinct values from the set.

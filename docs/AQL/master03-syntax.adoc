...
The `LIMIT` and `OFFSET` clauses are used to constrain the number of results from a query. With `LIMIT row_count`,
the result will have at most row_count items. The `OFFSET offset` is used to get results in a paginated way. For instance,

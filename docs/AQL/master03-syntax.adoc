...
Most of these functions are mapped to function definitions in the {openehr_foundation_types}[openEHR Foundation Types^].

There are two types of functions: _single-row_ functions, returning a single result for every row, and _aggregate_ functions, returning a single result based on a group of rows rather than on a single row.

NOTE: The followings are a set of core built-in functions defined by AQL specifications. Various other functions may exist however in AQL implementation.
...
Input values are selected by an _expression_ in a form of an <<_identified_paths, identified path>>.
...
AQL is providing single-row functions for examining and manipulating string values.
...
AQL is providing single-row functions for examining and manipulating date, time or datetime values.
...
AQL expressions provide a formal way of evaluating, transforming or comparing values. An expression is a combination of one or more <<_constants, constants>>, <<_operators, operators>>, <<_variables, variables>>, <<_predicates, predicates>>, <<_parameters, parameters>> and <<_functions, functions>> that evaluates to a value.

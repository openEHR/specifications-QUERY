...
These types are treated as strings and should comply with the rules for string quoting. The format of the date, time and datetime types should comply with the {iso_8601}[ISO 8601 Date and Time] format specification. Complete or partial values are allowed.
...
Then, the expressions in the `WHERE` clause will filter data, but only from the subset defined in the `FROM`, to get only the data that we need. And in the `SELECT` clause, a final projection is applied, selecting only the matched data that should be retrieved. In addition to the filtering done in the `WHERE` clause, the `class expressions` could hold predicates that also help filtering data.

Ass a summary:

. `FROM`: Defines the subset of data in which the query will be executed
. `WHERE`: Filters data from the subset, leaving only the data that we need

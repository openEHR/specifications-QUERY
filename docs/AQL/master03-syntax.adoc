...
[[_select_syntax]]
==== Syntax
The syntax always starts with the keyword `SELECT`, optionally followed by `TOP` (deprecated - see <<TOP, below>>), followed by <<_identified_paths,identified path(s)>> or plain <<_variables,variable>> name(s) defined in the `FROM` clause.

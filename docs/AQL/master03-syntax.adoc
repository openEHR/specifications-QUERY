...
[.deprecated]
*Deprecated*: Starting with Release 1.1.0, the use of `TOP` modifier is deprecated in favour of the `LIMIT` clause combined with `ORDER BY`.
...
The `LIMIT` and `OFFSET` clauses are used to constrain the result sets generated by the rest of the query.
...
NOTE: The use of `LIMIT` in combination with `ORDER BY` clause is recommended instead of the deprecated `TOP` alternative.
...
--------
Both `row_count` and `offset` are positive integer numbers, `row_count` minimal value is 1, while minimal value for `offset` is 0.
...
The following example will return 10 rows, representing the 2nd page of result sets, order by event start_time:
--------
SELECT
   TOP c/name/value AS Name, c/context/start_time AS date_time, c/composer/name AS Composer
FROM
   EHR e[ehr_id/value=$ehrUid]
      CONTAINS COMPOSITION c

...
The syntax of containment constraint expression is very simple: using `CONTAINS` operator between two class expressions. Left operand class expression is the parent object of the right operand class expression, e.g.
...
A column expression is formed by an <<_identified_paths,identified path>>, a <<_functions,function>>, a constant value or plain <<_variables,variable>> name defined in the `FROM` clause.
A constant is a fixed primitive type value (see <<_built_in_types>>), where strings, dates, times and datetimes should be quoted, numbers and booleans are not quoted.
